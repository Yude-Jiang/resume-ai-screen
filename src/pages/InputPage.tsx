      ...
      <div className="w-full md:w-1/2 border-r border-slate-200 overflow-y-auto bg-white/50 backdrop-blur-xl p-10 space-y-12 st-scrollbar">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-base font-black text-slate-400 uppercase tracking-widest">{t.jobDescription}</h3>
          ...
          <div className="relative group">
            <textarea
              ...
              className="w-full h-80 p-6 bg-white border-2 border-slate-100 rounded-[2rem] text-base focus:outline-none focus:border-st-light/50 transition-all shadow-sm hover:shadow-md st-scrollbar font-bold text-slate-600 resize-none leading-relaxed"
            />
            ...
            <button
              ...
              className={`absolute bottom-6 right-6 flex items-center gap-3 px-6 py-4 rounded-2xl text-base font-black tracking-widest shadow-xl transition-all active:scale-95 ${jd.trim() ? 'bg-st-dark text-st-yellow hover:bg-black' : 'bg-st-light text-white hover:bg-st-dark'}`}
            >
          ...
          <p className="text-sm text-slate-400 font-bold leading-relaxed">{t.jdHelpText || "AI will analyze JD to suggest weights and items."}</p>
        </div>
        ...
        <div className="space-y-8 pt-8 border-t border-slate-100">
           <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-black text-st-dark tracking-tight">{t.hardThresholds}</h3>
           </div>

           <div className="space-y-3">
              <label className="text-sm font-black text-slate-400 uppercase tracking-widest">Job Title</label>
              <input
                type="text"
                value={jobTitle}
           ...
           <div className="grid grid-cols-2 gap-8">
              <div className="space-y-3">
                 <label className="text-sm font-black text-slate-400 uppercase tracking-widest">{t.minEdu}</label>
                 <select 
                    value={thresholds.minEdu}
                    onChange={(e) => setThresholds({ ...thresholds, minEdu: e.target.value as EduLevel })}
                    className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm font-bold text-st-dark focus:outline-none focus:ring-2 focus:ring-st-light/20 transition-all appearance-none cursor-pointer"
                 >
              ...
              <div className="space-y-3">
                 <label className="text-sm font-black text-slate-400 uppercase tracking-widest">{t.minExp}</label>
                 <div className="relative">
                    <input 
                       ...
                       className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm font-bold text-st-dark focus:outline-none focus:ring-2 focus:ring-st-light/20 transition-all"
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm font-bold text-slate-400">{t.years}</span>
                 </div>
              </div>
           </div>
           ...
           <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2 min-h-[50px] p-4 bg-slate-100 rounded-2xl border-2 border-dashed border-slate-200">
                 {thresholds.requiredSkills.length === 0 ? (
                    <span className="text-xs text-slate-400 font-bold m-auto">{t.noThresholdAdded}</span>
                 ) : (
                    thresholds.requiredSkills.map((skill, idx) => (
                       <span key={idx} className="bg-white px-3 py-1.5 rounded-lg text-xs font-bold text-st-dark shadow-sm border border-slate-200 flex items-center gap-2 group animate-in zoom-in">
                          {skill}
                          <button 
              ...
              <div className="flex gap-2">
                 <input 
                    ...
                    className="flex-1 bg-white border border-slate-200 rounded-xl p-3 text-sm font-bold text-st-dark focus:outline-none focus:ring-2 focus:ring-st-light/20 transition-all shadow-inner"
                 />
              </div>
           </div>
        </div>
      ...
      <div className="w-full md:w-1/2 p-10 bg-slate-50/50 flex flex-col h-full st-scrollbar relative">
          ...
          {isAnalyzing && (
            ...
            >
              ...
              <div className="w-full max-w-md space-y-3">
                <div className="flex justify-between text-sm font-black tracking-widest text-slate-400 uppercase">
                ...
                <p className="text-sm italic text-slate-400 font-bold">
        ...
        <div className="mb-10 flex items-center justify-between">
           ...
           {files.length > 0 && (
              <button 
                onClick={() => setFiles([])}
                className="text-sm font-black text-rose-500 hover:text-rose-600 transition-colors bg-rose-50 px-4 py-2 rounded-full uppercase tracking-widest shadow-sm"
              >
                   ...
                   {files.map((f, i) => (
                     <div key={i} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3 group hover:border-st-light/30 transition-all">
                       ...
                       <div className="min-w-0">
                         <div className="text-sm font-bold text-st-dark truncate uppercase tracking-tight">{f.name}</div>
                         <div className="text-xs text-slate-400 font-bold">PDF DOCUMENT</div>
                       </div>
                     </div>
                   ))}
                 </div>
