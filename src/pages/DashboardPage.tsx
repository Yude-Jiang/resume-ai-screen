    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-700 h-full overflow-y-auto st-scrollbar pb-20">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between border-b border-slate-200 pb-4">
          <h3 className="text-2xl font-semibold text-st-dark flex items-center gap-3">
            <Briefcase className="w-6 h-6 text-st-light" />
            {t.activeJobs}
          </h3>
          <button
             onClick={() => setActiveTab('input')}
             className="px-6 py-2 bg-st-dark text-white rounded-xl text-sm font-medium  hover:bg-st-light transition-colors shadow-lg shadow-st-dark/10"
          >
            {t.inputData}
          </button>
        </div>
            ...
            >
              <div>
                ...
                <div className="grid grid-cols-3 gap-4 border-y border-slate-50 py-6 mb-6">
                   <div className="text-center">
                     <div className="text-xl font-semibold text-st-dark tabular-nums">{job.stats?.total || 0}</div>
                     <div className="text-xs font-medium text-slate-400">{t.totalHandled.split(' ')[1] || 'Resumes'}</div>
                   </div>
                   <div className="text-center border-x border-slate-50">
                     <div className="text-xl font-semibold text-st-dark tabular-nums">{job.stats?.evaluating || 0}</div>
                     <div className="text-xs font-medium text-slate-400">{t.evaluating}</div>
                   </div>
                   <div className="text-center">
                     <div className="text-xl font-semibold text-st-success tabular-nums">{job.stats?.highMatch || 0}</div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-3 bg-slate-50 hover:bg-st-dark hover:text-white rounded-xl text-xs font-medium  transition-all text-slate-500 whitespace-nowrap"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden">
            ...
            <div className="text-sm font-medium text-slate-500 mb-3">{t.totalHandled}</div>
            <div className="flex items-end gap-5">
              <span className="text-2xl font-semibold text-st-dark tabular-nums">{totalSystemStats.results}</span>
              <div className="flex items-center gap-1.5 text-st-success text-xs font-medium mb-1 px-2 py-0.5 bg-st-success/10 rounded-full">
          ...
          <div className="bg-st-dark p-10 rounded-[2.5rem] shadow-2xl hover:shadow-st-dark/30 transition-all group relative overflow-hidden">
            ...
            <div className="text-sm font-medium text-white/50 mb-3">{t.hoursSaved}</div>
            <div className="flex items-end gap-2">
              <span className="text-2xl font-semibold text-st-light tabular-nums">{(totalSystemStats.results * 0.2).toFixed(1)}</span>
              <span className="text-base font-medium text-st-light/40 mb-0.5">h</span>
            </div>
          </div>
          ...
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm group hover:border-st-light/50 transition-all">
            <div className="text-sm font-medium text-slate-500 mb-3">{t.avgScore}</div>
            <div className="flex items-end gap-2">
              ...
              <span className="text-base font-medium text-slate-300 mb-0.5">%</span>
            </div>
          </div>

          {/* System Load Activity */}
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden">
            <div className="text-sm font-medium text-slate-500 mb-3">Active Job Pipeline</div>
            ...
            <div className="flex justify-between items-center mt-6">
              ...
              <span className="text-xs font-medium text-st-light bg-st-light/10 px-2.5 py-1 rounded-full">
                Active jobs
              </span>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 bg-st-dark p-10 rounded-[2.5rem] shadow-2xl flex flex-col relative overflow-hidden">
          ...
          <div className="text-sm font-medium text-white/40 mb-10">{t.funnelTitle}</div>
          
          <div className="flex-1 flex flex-col justify-center gap-6">
            {funnelData.map((d: any, i: number) => (
              <div key={i} className="space-y-3 group">
                <div className="flex justify-between items-end">
                  <span className="text-sm font-medium text-white/60 tracking-tight group-hover:text-white transition-colors">{d.name}</span>
                  <span className="text-2xl font-medium text-white tabular-nums">{d.value}</span>
                </div>
          ...
          <div className="mt-12 pt-8 border-t border-white/5 flex items-center gap-4">
            ...
            <div>
              <div className="text-base font-medium text-white tracking-tight">{t.aiStrategy}</div>
              <div className="text-sm font-medium text-st-light mt-0.5">V4.2 Optimized Engine</div>
            </div>
          </div>
        </div>
      </div>
