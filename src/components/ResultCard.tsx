                      ...
                      <div className="space-y-4">
                        {res.summary.highlights.map((h, i) => (
                          <div key={i} className="flex gap-4 group cursor-pointer" onClick={() => setSearchTerm(h.split(' ').slice(0, 3).join(' '))}>
                            ...
                            <span className="text-sm text-slate-600 font-normal leading-relaxed group-hover:text-st-light transition-colors">{h}</span>
                          </div>
                        ))}
                      </div>
                    </section>
                      ...
                      <div className="space-y-4">
                        {res.summary.gaps.map((g, i) => (
                          <div key={i} className="flex gap-4 group cursor-pointer" onClick={() => setSearchTerm(g.split(' ').slice(0, 3).join(' '))}>
                            ...
                            <span className="text-sm text-slate-500 italic font-normal leading-relaxed group-hover:text-st-light transition-colors">{g}</span>
                          </div>
                        ))}
                      </div>
                    </section>
                    ...
                    <div className="flex items-start gap-8 relative z-10">
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="flex gap-2 pt-2">
                          <div className="px-4 py-2 bg-white rounded-xl text-xs font-medium text-st-dark border border-slate-200 shadow-sm flex items-center gap-2"><Target className="w-4 h-4 text-st-light" />{res.overall_recommendation.fit_status.toUpperCase()}</div>
                        </div>
                      </div>
                    </div>
                  </section>
                    ...
                    <section className="pt-8 border-t border-slate-100">
                      ...
                      <div className="bg-slate-100/50 rounded-2xl p-6 flex flex-col md:flex-row gap-6 items-end">
                        ...
                        <div className="w-24 space-y-2">
                          <label className="text-xs font-medium text-slate-500">{t.score}</label>
                          <input type="number" className="w-full bg-white border border-slate-200 rounded-xl p-3 text-sm font-normal text-st-dark focus:outline-none focus:ring-2 focus:ring-st-light/20 transition-all"
